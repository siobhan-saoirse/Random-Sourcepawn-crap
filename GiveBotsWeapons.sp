...
public void player_inv(Handle event, const char[] ename, bool dontBroadcast) 
{
	...
	if (!g_bSuddenDeathMode && (!g_bMVM || (g_bMVM && GetConVarBool(g_hCVMVMSupport) && GetClientTeam(client) == _:TFTeam_Red)) && IsPlayerHere(client))
	{
...
public Action Timer_GiveWeapons(Handle timer, any data)
{
	...
	if (!g_bSuddenDeathMode && (!g_bMVM || (g_bMVM && GetConVarBool(g_hCVMVMSupport))))
	{
		...
		if (g_bMVM && GetClientTeam(client) == _:TFTeam_Red) { 
			...
			if (TF2_GetPlayerClass(client) == TFClass_Scout) {
				...
				switch(GetRandomInt(1,4))
				{
					case 1:
					{
						...
						TF2Attrib_SetByName(weapon[0], "faster reload rate", 0.22);
						...
						TF2Attrib_SetByName(client, "Projectile speed increased", 0.4);
				...
				switch(GetRandomInt(1,3))
				{
					...
					{
						TF2Attrib_SetByName(weapon[0], "fire rate penalty", 0.1);
						TF2Attrib_SetByName(weapon[0], "faster reload rate", 0.65);
						TF2Attrib_SetByName(weapon[0], "clip size upgrade atomic", 7.0);
						TF2Attrib_SetByName(weapon[0], "Projectile speed increased", 1.1);
						TF2Attrib_SetByName(weapon[0], "projectile spread angle penalty", 5.0);
					}
				}
				TF2Attrib_SetByName(weapon[1], "damage penalty", 1.5);
